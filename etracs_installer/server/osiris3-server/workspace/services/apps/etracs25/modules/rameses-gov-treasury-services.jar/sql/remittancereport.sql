...
FROM (
  SELECT
    ...
    CASE WHEN cv.objid IS NULL 
      THEN CONCAT( (CASE WHEN cri.qtr = 0 THEN 1 ELSE cri.qtr END),  'Q,' , MIN(cri.year),  ' - ',  CASE WHEN cri.qtr = 0 THEN 4 ELSE cri.qtr END , 'Q,' , MAX(cri.year))
      ELSE '' 
    END AS payperiod, 
    'BASIC' AS type, 
