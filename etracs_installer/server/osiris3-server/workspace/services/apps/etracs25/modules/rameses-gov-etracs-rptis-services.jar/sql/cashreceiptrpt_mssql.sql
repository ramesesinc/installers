...
SELECT
	t.*,
	(SELECT CONVERT(VARCHAR(1), MIN(CASE WHEN qtr = 0 THEN fromqtr ELSE qtr END)) FROM cashreceiptitem_rpt_online WHERE rptreceiptid = t.rptreceiptid AND year = t.minyear) + 
	'Q,' + CONVERT(VARCHAR(4),t.minyear) + ' - ' + 
	(SELECT CONVERT(VARCHAR(1), MAX(CASE WHEN qtr = 0 THEN toqtr ELSE qtr END)) FROM cashreceiptitem_rpt_online WHERE rptreceiptid = t.rptreceiptid AND year = t.maxyear) + 
...
[findPreviousReceipt]
SELECT DISTINCT TOP 1 cr.objid AS rptreceiptid, cr.txndate 
FROM cashreceipt cr 
	LEFT JOIN cashreceipt_void cv ON cr.objid = cv.receiptid 
